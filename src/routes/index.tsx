...
function Home() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-6 sm:py-10 space-y-8">
      {/* Hero */}
      <section className="grid gap-6 lg:grid-cols-[1.4fr_1fr] animate-slide-up">
        <div className="relative overflow-hidden rounded-2xl carbon-texture team-aura border border-border p-6 sm:p-10">
          {/* Circuit signature backdrop */}
          <div className="pointer-events-none absolute -right-6 -top-6 h-64 w-[28rem] text-accent opacity-25">
            <CircuitSignature id={circuit.id} className="h-full w-full" strokeWidth={1.4} />
          </div>
          <div className="pointer-events-none absolute inset-x-0 bottom-0 h-24 speed-stripes opacity-30" />
          ...
          <div className="relative">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-muted-foreground">
              ...
              {nextRace.hasSprint && (
                <span className="ml-1 rounded-full border border-accent/60 bg-accent/10 px-2 py-0.5 text-[10px] uppercase tracking-widest text-accent-glow">
                  <Zap className="inline h-3 w-3 -mt-0.5 mr-0.5" />Sprint weekend
                </span>
              )}
            </div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <Link
                to="/calendar"
                className="inline-flex items-center gap-2 rounded-full border border-border bg-surface/60 px-5 py-2.5 text-sm font-semibold uppercase tracking-widest hover:border-accent/50 hover:bg-surface transition-colors"
              >
                Full calendar
              </Link>
            </div>
        ...
        {/* Personalized module */}
        <div className="glass rounded-2xl p-6 flex flex-col hover-lift relative overflow-hidden">
          <div
            className="pointer-events-none absolute -right-12 -bottom-12 h-48 w-48 rounded-full opacity-20 blur-3xl"
          ...
          {favDriver && (
            ...
            >
              <div
                className="h-12 w-12 rounded-full border-2 flex items-center justify-center font-display transition-transform group-hover:scale-110"
                style={{ borderColor: team.color, background: `linear-gradient(135deg, ${team.color}20, ${team.color}55)` }}
              >
                {favDriver.code}
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Lead driver</div>
                <div className="font-display text-lg truncate story-link inline-block">{favDriver.firstName} {favDriver.lastName}</div>
              </div>
      ...
      <section className="grid gap-6 md:grid-cols-2">
            ...
            {top3D.map((s) => {
              ...
              return (
                <li key={s.driver.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0 group">
                  ...
                  <div className="w-1 h-8 rounded-full transition-all group-hover:h-10" style={{ background: t.color }} />
                  <div className="flex-1 min-w-0">
        ...
        >
          <ol className="divide-y divide-border">
            {top3C.map((s) => (
              <li key={s.team.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0 group">
                <div className="font-display text-3xl w-8 text-center" style={{ color: s.position === 1 ? "var(--accent)" : undefined }}>
                  {s.position}
                </div>
                <div className="w-1 h-8 rounded-full transition-all group-hover:h-10" style={{ background: s.team.color }} />
                <div className="flex-1 min-w-0">
...
function Stat({ label, value, small }: { label: string; value: string | number; small?: boolean }) {
  return (
    <div className="rounded-lg border border-border bg-surface/40 p-3 transition-colors hover:border-accent/40">
  ...
  return (
    <div className="glass rounded-2xl p-6 hover-lift">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link to={href} className="text-xs uppercase tracking-widest text-accent story-link">See all</Link>
      </div>
      {children}
    </div>
  );
...
function LastRaceCard() {
  ...
  return (
    <Link
      ...
      className="relative overflow-hidden glass rounded-2xl p-6 group hover:border-accent/50 transition-colors block hover-lift"
    >
      <div className="pointer-events-none absolute right-2 bottom-2 h-24 w-40 text-accent opacity-15">
